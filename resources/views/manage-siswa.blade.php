...
@section('content')
    <div class="card m-5">
        <div class="card-header flex-wrap d-flex align-items-center gap-2">
            <h3 class="card-title mb-0 me-4">Daftar Siswa</h3>
            <!-- Controls: search + add -->
            <div class="d-flex align-items-center gap-2 flex-nowrap my-3 ms-auto">
                <div class="input-group input-group-sm">
                    <span class="input-group-text"><i class="bi bi-search"></i></span>
                    <input type="text" id="studentsSearch" class="form-control form-control-sm" placeholder="Cari kata kunci...">
                </div>
                <button id="btnAddSiswa" class="btn btn-primary btn-sm text-nowrap">
                    ...
                    <tbody>
                        @foreach ($students as $s)
                            <tr data-id="{{ $s->id }}">
                                <td class="photo">
                                    @if ($s->photo_url)
                                        <img src="{{ $s->photo_url }}" alt="Foto" class="rounded" style="width:40px;height:40px;object-fit:cover;">
                                    @else
                                        <img src="https://ui-avatars.com/api/?name={{ urlencode($s->name) }}&background=random&color=fff&size=80&bold=true" alt="Avatar" class="rounded" style="width:40px;height:40px;object-fit:cover;">
                                    @endif
                                </td>
                                <td class="nis">{{ $s->nis }}</td>
                                <td class="name">{{ $s->name }}</td>
                                <td class="class">{{ $s->class ?? '-' }}</td>
                                <td class="text-end">
                                    <button class="btn btn-sm btn-light-primary btnEdit" data-id="{{ $s->id }}">
                                        <i class="bi bi-pencil-square"></i>
                                    </button>
                                    <button class="btn btn-sm btn-light-danger btnDelete" data-id="{{ $s->id }}">
...
@push('scripts')
    ...
    <script>
        ...
        const routes = {
            index: '{{ route('siswa.index') }}',
            store: '{{ route('siswa.store') }}',
            show: (id) => '{{ route('siswa.show', ['id' => 'ID_PLACEHOLDER']) }}'.replace('ID_PLACEHOLDER', id),
            update: (id) => '{{ route('siswa.update', ['id' => 'ID_PLACEHOLDER']) }}'.replace('ID_PLACEHOLDER', id),
            destroy: (id) => '{{ route('siswa.destroy', ['id' => 'ID_PLACEHOLDER']) }}'.replace('ID_PLACEHOLDER', id),
        };
        ...
        $('#studentsTable').on('click', '.btnEdit', function() {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    const uiAvatar = (name) => `https://ui-avatars.com/api/?name=${encodeURIComponent(name || 'Siswa')}&background=random&color=fff&size=160&bold=true`;
                    const imgUrl = d.photo_url || uiAvatar(d.name);
                    photoWrapper.style.backgroundImage = `url('${imgUrl}')`;
                    if (photoRemove) photoRemove.value = '';
                },
                error: function() {
                    toastr?.error?.('Gagal memuat data siswa. Coba lagi.');
                    studentModal.hide();
                },
                complete: function() {
        ...
        form.addEventListener('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    const d = resp.data;
                    const uiAvatar = (name) => `https://ui-avatars.com/api/?name=${encodeURIComponent(name || 'Siswa')}&background=random&color=fff&size=80&bold=true`;
                    ...
                    if (!id) {
                        ...
                        $(rowNode).children().eq(1).addClass('nis');
                        $(rowNode).children().eq(2).addClass('name');
                        $(rowNode).children().eq(3).addClass('class');
                    } else {
                        const $row = $(`#studentsTable tr[data-id='${id}']`);
                        $row.find('.photo').html(photoCell);
                        $row.find('.nis').text(d.nis);
                        $row.find('.name').text(d.name);
                        $row.find('.class').text(d.class || '-');
                    }
                    studentModal.hide();
